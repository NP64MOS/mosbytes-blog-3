...
export async function updatePost(id, updateData) {
  ...
  if (useNeonDatabase) {
    try {
      ...
      console.log('Database adapter - updatePost result:', result)
      // If we get a result object, the update was successful
      if (result && typeof result === 'object') {
        return true
      }
      // If we get null but no error, the post might not exist
      return false
    } catch (error) {
      console.error('Neon error, falling back to JSON:', error)
...
export async function deletePost(id) {
  ...
  if (useNeonDatabase) {
    try {
      const result = await deletePostNeon(id)
      console.log('Database adapter - deletePost result:', result)
      // If we get a boolean result, return it
      if (typeof result === 'boolean') {
        return result
      }
      // If we get something else, assume success if not null/undefined
