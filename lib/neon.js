...
export async function updatePostNeon(id, updateData) {
  try {
    const result = await sql`
      ...
      RETURNING *
    `
    console.log('Update result:', result, 'Type:', typeof result, 'Length:', result?.length)
    // If RETURNING clause returns data, the update was successful
...
export async function deletePostNeon(id) {
  try {
    ...
    console.log('Delete result:', result, 'Type:', typeof result, 'Length:', result?.length)
    // Neon PostgreSQL returns an array with rowCount
